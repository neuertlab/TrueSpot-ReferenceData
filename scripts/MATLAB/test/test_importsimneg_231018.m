...
UseDir = [ImgProcBaseDir filesep 'img' filesep 'simneg'];
...
for r = 1:imgcount
    ...
    simparam.spots_seed = 0;
    simparam.spots_actual = 0;
    simparam.amplitude_mean = 0;
    simparam.amplitude_var = 0;
    simparam.bg_level = getTableValue(ImageTable, r, 'BGLevel');
    simparam.bg_var = getTableValue(ImageTable, r, 'BGVar');

    %Save
    imgdat = imgdat16;
    save([UseDir filesep 'simneg_' simid '.mat'], 'imgdat', 'simparam', 'imgdat_raw');
    
    tifpath = [TifDir filesep 'simneg_' simid '.tif'];
    if isfile(tifpath)
        delete(tifpath);
    end
