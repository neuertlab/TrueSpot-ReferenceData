...
function fig_handle = GenMultiTool_ROCPlot_MaxProj(img_summary, zMin, zMax, figno)
...
if isfield(img_summary, 'results_hb')
    PlotIndivPRCurve_MaxProj(img_summary.results_hb, COLOR_HB, LINE_WIDTH, LINE_STYLE, zMin, zMax);
    xlim([0.0 1.0]);
    ylim([0.0 1.0]);
...
if isfield(img_summary, 'results_bf')
    PlotIndivPRCurve_MaxProj(img_summary.results_bf, COLOR_BF, LINE_WIDTH, LINE_STYLE, zMin, zMax);
    xlim([0.0 1.0]);
    ylim([0.0 1.0]);
    fig_handle_temp = fig_handle;
else
