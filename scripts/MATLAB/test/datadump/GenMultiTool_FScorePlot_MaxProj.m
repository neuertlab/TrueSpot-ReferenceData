...
function fig_handle = GenMultiTool_FScorePlot_MaxProj(img_summary, zMin, zMax, figno)
...
if isfield(img_summary, 'results_hb')
    fig_handle_temp = PlotIndivFScoreCurve_MaxProj(img_summary.results_hb, COLOR_HB, LINE_WIDTH, LINE_STYLE, fig_handle, zMin, zMax);
else
    fig_handle_temp = [];
end
...
if isfield(img_summary, 'results_bf')
    fig_handle_temp = PlotIndivFScoreCurve_MaxProj(img_summary.results_bf, COLOR_BF, LINE_WIDTH, LINE_STYLE, fig_handle, zMin, zMax);
else
    fig_handle_temp = [];
end
