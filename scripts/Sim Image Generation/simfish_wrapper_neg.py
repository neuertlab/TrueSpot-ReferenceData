...
def main(args):
    outdir = "D:\\usr\\bghos\\labdat\\imgproc\\img\\simneg"
    ...
    outfile.write("#ID,BGLevel,BGVar\n")
    for i in range(genamt):
        ...
        bglvl = random.uniform(bg_level_min, bg_level_max)
        bgvar = random.uniform(bg_var_min, bg_var_max)
        res = simfish.simulate_image(3, 0, random_n_spots=False, n_clusters=cluster_count, random_n_clusters=True, n_spots_cluster=spots_per_clust, random_n_spots_cluster=True, image_shape=(dim_z, dim_xy, dim_xy), subpixel_factors=None, voxel_size=(voxel_z, voxel_xy, voxel_xy), sigma=(ptsig_z, ptsig_xy, ptsig_xy), random_sigma=random_sig, amplitude=0, random_amplitude=0, noise_level=bglvl, random_noise=bgvar)
        saveSimImage(res, outdir, idstr)
        outfile.write(idstr + "," + str(bglvl) + "," + str(bgvar) + "\n")
    outfile.close()
    print("hold")
