...
**TODO** Experimental data refset organization was tidied up a bit in Decemberish. Update this section and figures accordingly.
...
The summary table for the experimental images with reference sets generated by B.H. is included as `tables/expBHStatsDump_231215.tsv`, and was generated using a version of `bp_fullset_230612.m` that called `openExpDumpOutput` and `Dump_expResultStats`.

`expBHStatsDump_231215.tsv` was the table used for figure generation and statistics, as can be seen in `scripts/Figure Generation/expResults.R`.

### Per-Cell Counts
Per-cell counts were gathered from the callset/results files using the `dump_percell_probdistro_231205.m` script. The resulting table is `exp_percell_counts_231215.csv`. The yeast timecourse data was dumped using a different script `dump_percell_probdistro_sctc_231212.m` to `exp_sctc_percell_counts_stats.tsv`. The difference in scripts is mostly for organization of output data. These scripts also generate the raw probability distribution graphs.

### Threshold Call Table
The table containing the dump of all threshold selections is included as `tables/thstatsdump_240112.tsv`. This table was used for all figure generation. The dump table was generated using a version of `bp_fullset_230612.m` that called `openThOutput` and `Dump_ThreshTable`.
...
Curves were generated from callset data using either `run_dump_all_plots_230217.m` for batches or `run_dump_indivplots_230503.m` for individual images. `dump_group_prcs_240126.m` was used for the group composite PR curves shown in Figure 2. Raw dump svgs were cleaned up and combined into larger figures using Inkscape. Most raw dump svgs that were used for figures are provided in this repository.
