...
### Maximum Intensity Projection Spot Calling -- TrueSpot
Images were collapsed to maximum intensity projections (MIPs) prior to running any spot detection (`test_gen_maxproj_240328.m`). In the case of "trimmed" stacks, edge slices were removed (or zeroed-out) before maximum was taken. Cluster jobs were generated by `gen_hb_runs_maxproj_240505.m`. Importantly, the -maxzproj mode for TrueSpot was NOT used as this mode collpases the image post-filtering and we wanted to use the MIPs of the raw images.

As this analysis was done later, raw TrueSpot output files are in the less-redundant newer format.
...
MIPs were run with the same setup, with the MIP tif being provided as input and no z voxel size or point size estimate specified. `gen_bf_runs_maxproj_240328.m` was used to generate batch jobs.
...
### MIP Call Import -- TrueSpot & Big-FISH
MIP callsets from TrueSpot and Big-FISH were imported into the composite results files via `test_import_mipresults_240411.m`. This script also tested the callsets against reference sets, when present. Reference sets were collapsed to 2D for comparison and in cases of trimmed stacks, all reference spots more than 2 slices from the z borders were removed. The same collapse method was used to compare 3D and 2D counts.
...
Below is an example of a `results_hb` struct from an experimental image.
...
The summary table used to run statistical tests and generate figures from the results of benchmarking tests on simulated images is `tables/sim_results_240426.csv`. This file was generated (the same process is used for regeneration) by running the `run_dump_allsim_230424.m` script, which cycles through the results `mat` files and dumps pertinent information into a plain text table. Simulation parameters for the earlier simulated images and RS-FISH benchmarking set had to be re-added manually as they are not stored in the results mat files. These parameters can be found in the included `sim_results_240426.csv` and in one of the supplementary tables in the manuscript.
...
The summary table for the experimental images with reference sets generated by B.H. is included as `tables/expBHStatsDump_240429_1.tsv`, and was generated using a version of `bp_fullset_230612.m` that called `openExpDumpOutput` and `Dump_expResultStats`.

`expBHStatsDump_240429_1.tsv` was the table used for figure generation and statistics, as can be seen in `scripts/Figure Generation/expResults.R`.

### Per-Cell Counts
Per-cell counts were gathered from the callset/results files using the `dump_percell_probdistro_231205.m` script. The resulting table is `exp_percell_counts_240502.csv`. The yeast timecourse data was dumped using a different script `dump_percell_probdistro_sctc_231212.m` to `exp_sctc_percell_counts_stats.tsv`. The difference in scripts is mostly for organization of output data. These scripts also generate the raw probability distribution graphs.

### Threshold Call Table
The table containing the dump of all threshold selections is included as `tables/thstatsdump_240503.tsv`. This table was used for all figure generation. The dump table was generated using a version of `bp_fullset_230612.m` that called `openThOutput` and `Dump_ThreshTable`.
...
The table containing the number of spots counted per cell (using group fixed and per-image thresholds) can be found in `tables/sctcdump_cell_240429.tsv`, and was generated from the per-image results files by a version of `bp_fullset_230612.m` that called `open_sctcOutput`, `close_sctcOutput`, `do_sctcIndiv`, and `Dump_sctcStats`. `sctcdump_sim_240429.tsv` was also produced, and it contains the TrueSpot and Big-FISH counts from the simytc sets. 

### MIP Run Data
Maximum intensity projection spot counts broken down per cell can be found in `maxprojResults_240510`, a table generated by `dump_maxprojResults_240430.m`. Code used to generate all related figures can be found in `expResults.R`, including for yeast time courses.
...
###  Per-Cell Count Correlations & Line Fitting
When comparing per-cell spot counts for TrueSpot and Big-FISH, we ran correlation calculations similar to with the simulated parameter correlations (however, as the expected relationship was should be linear, we chose to examine the Pearson estimates over the Spearman estimates). Additionally, because high correlation between tool counts by itself is not necessarily indicative of high agreement (ie. cases where one tool consistently picks about twice the number of spots as the other would yield a high correlation estimate), we also ran a linear fit for each subgroup and examined the resulting slope.

The code can be found in `expResults.R`.

```
inputTablePath <- "(DIRECTORY)\\tables\\exp_percell_counts_231215.csv"
...
The code for calculating the simytc actual to detected spot count correlations can be found in `yeast_tc_stats.R`.

```
#--- Sim actual vs. detected Stats
inputTablePath <- "(DIRECTORY)\\tables\\sctcsim_counts_231013.tsv"
...
In order to statistically quantify similarity between time course curves (comparing tools, tools to reference, or threshold averaging methods), I decided to try a paired t-test using each time point value between two compared curves as pairs. The relevant code can be found in the `Stats` section of `yeast_tc_stats.R`.

### Threshold Variation Measurements
The `th_stats.R` script contains all of the code for generating the figures and running stats for evaluating threshold variability.

## Figure Generation
This section describes how figures were generated from data tables.

### Concept Diagrams
The concept diagrams showing idealized curve shapes and describing internal pipeline flow were generated using [BioRender](https://www.biorender.com) (Figure 1J-P) or drawing lines and shapes by hand in vector image editor Inkscape.

### Data Curves (Spot count, F-Score, and PR)
Curves were generated from callset data using either `run_dump_all_plots_230217.m` for batches or `run_dump_indivplots_230503.m` for individual images (`run_dump_indivplots_exp_240415.m` was used for experimental images after results save format was tweaked). `dump_group_prcs_240126.m` was used for the group composite PR curves shown in Figure 2. Raw dump svgs were cleaned up and combined into larger figures using Inkscape. Most raw dump svgs that were used for figures are provided in this repository. Slightly modified scripts were used to dump curves for MIPs (due to the slightly different storage format within the results files) - see `run_dump_all_plots_maxproj_240509.m`

### Simulated Image Result Violin Plots
All violin plots illustrating maximum recall, PR-AUC, or F-Score distributions for simulated images were generated using `simResults.R` and saved to svg using the RStudio plotting interface. The raw output svgs were cleaned up (coloring or texturing data sets, adjusting font size and scale for axis labels, labeling component groups, etc.) and combined in Inkscape.

### Experimental Image Metrics Box-and-Whisker Plots
Similar to with simulated images, the box-and-whisker plots illustrating maximum recall, PR-AUC, and F-Score distributions for experimental images were generated using `expResults.R`, and the resulting plots were exported to svg for cleanup and combination.

### Probability Distribution Plots
Probability distribution plots were generated in MATLAB and exported to svg using the `dump_percell_probdistro_231205.m` and `dump_percell_probdistro_sctc_231212.m` scripts. For the sctc plots, a shaded error region is defined for each set. In the 0.2M experiment, which had 2 biological replicates, this region was simply the shading between the two lines for each replicate. The 0.4M experiment had 3 replicates, so the lines indicate the average of the 3 and the shaded region represents the standard deviation. Lines and regions for RS-FISH (green) and deepBlink (yellow) were generated for these graphs as well using our automated thresholder on RS-FISH and a hard 95% probability cutoff for deepBlink, but due to visual cluttering and because these weren't exactly the same as comparing the automated thresholding results of the two other tools, these layers were set to invisible and removed from the final rendering.

### Tool Count Correlation Plots
The count correlation plots were generated using code in `expResults.R`, including those for the yeast time course, and exported to svg. Post export cleanup involved updating the color coding and combining the scatterplots for experimental subgroups and sctc biological replicates. The color palette was chosen to minimize conflict with the tool color coding and the sctc time course color coding. All count correlation plots for MIPs, including for yeast time course, were also generated from code in `expResults.R`.

### Parameter Correlation Heatmaps
These, like the violin plots, were generated using `simResults.R`. For these we used ggplot2 with `geom_density_2d_filled(bins = 50)` and the default viridis color scale. The legend generated by `geom_density_2d_filled` had an entry for each bin making it impractical to include as-is. Thus, in the supplementary figures a continuous legend using the values pulled from the respective legends is used instead. The "level" values used are related to density, but have some dependency on the data range. Thus, it is best to interpret these relative to themselves or plots generated as different facets in the same call (such as PR-AUC versus SNR for different tools).

### Simulated Image Performance Heatmaps
The rainbow ("turbo") heatmaps of performance scores for individual simulated images were generated in MATLAB using the `run_gen_moreheatmaps_230605.m` script.

### Detected vs. Actual Spot Counts for Simulated Images
The simple detected vs. actual spot counts for TrueSpot and Big-FISH across all simulated images plots in Figure S8 were generated in MATLAB using `run_boring_scatterplots_230530.m` (they are "boring" because the colors were removed). The scatterplots for detected vs. actual spot counts for simytc were also generated in MATLAB, but instead using `run_sctcsim_varfixedplots_230627.m`.

### Threshold Variability Box-and-Whisker & CoV Bargraphs
Both the normalized threshold plots and the CoV plots were generated in `th_stats.R`.

### Time Course ON Proportion & Avg. per ON Plots
